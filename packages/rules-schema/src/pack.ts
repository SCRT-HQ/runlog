...
export const Vocabulary = z
  .object({
    ...
    finalize: z.string().default("Finalize").describe("Verb for closing a unit, e.g. Finalize, Fire, Commit."),
    /**
...
export const StateDef = z
  .object({
    ...
    short: z
      ...
      .max(8)
      .optional()
      .describe("Compact form for embedding in the name of the thing itself, a file, a layer, a DAW track. Keep it very short, e.g. VF."),
    scope: z
...
export const CounterDef = z
  .object({
    ...
    hidden: z.boolean().default(false).describe("Track it without showing it, for tension the player should feel but not read."),
      ...
      .array(
        z
          .object({
            ...
            oncePerRun: z.boolean().default(false).describe("Fire at most once per run, however often the threshold is met."),
...
export const ResourceDef = z
  .object({
    ...
    display: z.enum(["boxes", "bar", "number"]).default("number").describe("How to draw it: tick boxes, a filled bar, or a plain number."),
    description: z.string().optional().describe("What it represents and how it is spent."),
  })
  .strict()
  .describe("A numeric track the player fills or spends: a progress clock, a word count, training volume, stock on hand.");
...
export const Clock = z
  .object({
    ...
    minutes: z
      ...
      .max(24 * 60)
      .optional()
      .describe("How long a timer runs. Required for a timer; ignored by a stopwatch."),
    label: z.string().optional().describe("What the clock is called on screen. Defaults to the unit's name and number."),
    auto: z
      .boolean()
      .default(true)
      .describe(
        "Start it when the unit is entered and stop it when the unit closes. Off, the player starts it by hand; it still stops with the unit.",
      ),
...
const ScoreCommon = {
  better: z
    .enum(["higher", "lower"])
    .optional()
    .describe("Which way wins: a higher number or a lower one. Defaults to higher, except for time, which defaults to lower."),
  label: z
    .string()
    .min(1)
    .optional()
  ...
  tiebreak: z
    .enum(["time", "units"])
    .optional()
    .describe("A second key to settle a tie on the first: time (the shorter wins) or units closed (the greater wins)."),
};
    ...
    z
      .object({
        ...
        optional: z
          ...
          .optional()
          .describe(
            "May be left unticked: closing the unit does not wait for it. For boxes that record an outcome rather than promise one.",
          ),
  ...
  .discriminatedUnion("kind", [
    z
      .object({
        ...
        optional: z.boolean().default(false).describe("Let the player skip this roll rather than requiring it."),
        skipWhen: z.array(Predicate).optional().describe("Reasons to skip this step. Any one of them is enough."),
      })
      .strict()
    ...
    z
      .object({
        ...
        constrainedBy: Id.optional().describe("Table whose result, if rolled this unit, constrains what may be declared."),
        skipWhen: z.array(Predicate).optional().describe("Reasons to skip this step. Any one of them is enough."),
      })
      .strict()
    ...
    z
      ...
      .strict()
      .describe("A step the engine cannot perform or verify, the actual creative work. It only tracks that you did it."),
    z
      .object({
...
export const Phase = z
  .object({
    ...
    skipWhen: z
      ...
      .optional()
      .describe(
        "Reasons to skip this phase. Any one of them is enough: a list of two means skip if either applies. Omit it to always play the phase.",
      ),
  })
  .strict()
...
export const TargetingDef = z
  .discriminatedUnion("strategy", [
    ...
    z
      .object({ strategy: z.literal("oldest").describe("Always hit the oldest eligible subject.") })
      .strict()
      .describe("Always hit the oldest eligible."),
    z
      .object({ strategy: z.literal("newest").describe("Always hit the newest eligible subject.") })
      .strict()
      .describe("Always hit the newest eligible."),
    z
      .object({
          ...
          .array(
            z
              .object({
                ...
                anchor: z.enum(["newest", "oldest", "playerChoice"]).describe("Which subject counting starts from."),
                direction: z
                  .enum(["before", "after"])
                  .optional()
        ...
        offsetFrom: z.enum(["onesDigit"]).default("onesDigit").describe("How the step count is read out of the roll."),
        wraparound: z.boolean().default(true).describe("Wrap past the ends of the list rather than stopping."),
        skipIneligible: z.boolean().default(true).describe("Skip untargetable and incomplete subjects while counting."),
        eventFallback: z
          .object({
            roll: DiceExpr.describe("What to roll to pick a band."),
            tensOddBand: z.number().int().describe("Band to use when the tens digit is odd, given as the band's low value."),
            tensEvenBand: z.number().int().describe("Band to use when the tens digit is even, given as the band's low value."),
            missOn: z
              .number()
              .int()
              .optional()
          ...
          .strict()
          .optional()
          .describe("How to pick a target when the consequence was not triggered by a roll that already selects a band."),
      })
      .strict()
      .describe(
...
export const EndingDef = z
  .object({
    ...
    requires: z.array(Predicate).optional().describe("Conditions under which this ending is available."),
  })
  .strict()
  ...
  .object({
    ...
    players: z
      .object({
            ...
            z
              .object({
                ...
                acts: z
                  ...
                  .default(false)
                  .describe(
                    "Whether this role takes the table's actions in a unit it holds: rolling, declaring, ticking, closing. Where no role declares it, any seat acts.",
                  ),
              })
              .strict()
        ...
        rotate: z.enum(["none", "clockwise"]).default("none").describe("Whether roles pass to the next player when a unit closes."),
      })
      .strict()
      .optional()
      ...
      .array(
        z
          .object({
            ...
            extra: z.array(Action).optional().describe("Extra actions run when this unit begins, after any skips are applied."),
          })
          .strict()
          .describe("An override for one unit."),
      )
      .optional()
      .describe("Per-unit overrides, for modes with a fixed shape,'unit three always suffers a consequence', and the like."),
...
export const MoveDef = z
  .object({
    ...
    when: z
      ...
      .default("anytime")
      .describe("When the move is offered: at any point, only between units, or only when the player is deciding whether to stop."),
  })
  .strict()
    ...
    z
      .object({
        step: z
          ...
          .min(1)
          .describe(
            'The active step: a phase id, or "phaseId#index" for a phase with more than one step. Must be the step the engine is actually waiting on.',
          ),
        answers: PlayAnswers.optional(),
      })
      .strict()
    ...
    z
      .object({
        ...
        play: z
          .array(PlayStep)
          .min(1)
          .describe("The script to play through: entering units, declaring subjects, rolling tables, taking moves."),
...
export const Issue = z
  .object({
    to: z.string().min(1).describe("Who this copy was issued to. Shown to them in the app and stamped into anything they export."),
    reference: z
      .string()
      .optional()
      .describe(
        "The seller's own reference for the sale, e.g. an order id. Lets a leaked copy be traced without the name being the only link.",
      ),
...
export const Signature = z
  .object({
    ...
    signedBy: z.string().optional().describe("What the signer calls themselves. A claim carried by the pack, not proof of identity."),
  })
  .strict()
  .describe("Proof that a pack's contents are unchanged since its author signed them. Not a restriction on copying.");
...
export const License = z
  .object({
    ...
    text: z
      ...
      .optional()
      .describe(
        "The license or notice in full, shown with the pack and printed in its rulebook. Required for `proprietary` and `custom`; welcome on any.",
      ),
    redistributable: z
      .boolean()
      .describe(
...
export const Pack = z
  .object({
    $schema: z.string().optional().describe("Path or URL to this schema, so your editor offers completions and validation."),
    schemaVersion: z
    ...
    category: z
      ...
      .optional()
      .describe(
        "What kind of thing this is, in a word or two, for a marketplace to group by: everyday, games, craft, fitness, cooking, writing…",
      ),
    tags: z
      ...
      .optional()
      .describe(
        "Free tags for a marketplace to filter by: the game it is for, the hobby, the shape of play. Short, and in the words a person would search for.",
      ),
    use: z
      .array(
        z
          .object({
            ...
            merge: z
              ...
              .default("replace")
              .describe(
                "`replace` uses the borrowed entries alone; `append` puts them after this pack's own table of that id, which is how a pack keeps its results and borrows more.",
              ),
          })
          .strict(),
      )
        ...
        z
          .object({
            label: z
              ...
              .max(80)
              .optional()
              .describe("What to call it on screen where something has to name it; the tag itself, otherwise."),
            note: z
              .string()
              .max(200)
              .optional()
              .describe("What carrying this mark means, for whoever is writing something that matches on it."),
          })
          .strict(),
      )
      .optional()
    ...
    extends: PackId.optional().describe("Base pack this one layers on, for house rules and variants."),
    ...
    unit: z
      .object({
        ...
        createsSubject: z.boolean().default(true).describe("Whether entering a unit produces a new subject."),
        handsFree: z
          .boolean()
          .default(false)
          .describe(
    ...
    states: z.record(Id, StateDef).optional().describe("Persistent states that can attach to subjects or the run, keyed by id."),
    ...
    phases: z.array(Phase).min(1).describe("The per-unit flow, in order. At least one phase must close the unit."),
    moves: z
      .record(Id, MoveDef)
      .optional()
      .describe(
    ...
    triggers: z.array(Trigger).optional().describe("Global triggers not owned by any table entry, e.g. something that fires every unit."),
    ...
    asks: z
      .object({
        policy: z.enum(["ask", "auto"]).describe("`ask` puts one in front of the table to accept; `auto` takes it as it lands."),
      })
      .strict()
      .optional()
      .describe(
  ...
  .strict()
  .describe("A complete, self-contained description of a dice-driven creative-practice game. Packs are data, never code.");
