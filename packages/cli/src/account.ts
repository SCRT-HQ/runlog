...
import { configDir as sessionConfigDir, deviceFlow, needsRenewal, realDeps, renew, type FlowDeps, type Session } from "@runlog/session";
...
/** The CLI's own config directory: `RUNLOG_CONFIG_DIR`, else the platform's rule, for `runlog`. */
function configDir(): string {
  return process.env["RUNLOG_CONFIG_DIR"] ?? sessionConfigDir("runlog");
}
...
export async function bearer(creds: Credentials, force = false, deps: FlowDeps = realDeps): Promise<string> {
  ...
  const lapsing = !creds.session.expiresAt || needsRenewal(creds.session, deps.now());
